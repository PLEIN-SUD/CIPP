...
// Captures the incident-management facts the collection cannot know, so the incident report is
// generated from a record rather than hand-written. Only appears once a compromise is retained:
// before that, the document it feeds has no reason to exist.
...
export const PsitBecIncidentPanel = ({ userData, becData, tenantFilter, triage = [] }) => {
  ...
  const saveRequest = ApiPostCall({ relatedQueryKeys: [`PSITBecIncident-${tenantFilter}-${userId}`] })
  ...
  const exposure = useMemo(() => buildExposure(becData, signals, triage), [becData, signals, triage])
  ...
  const removeRow = (field, index) => set(field, list(field).filter((_, position) => position !== index))
        ...
        <Stack spacing={2} divider={<Divider flexItem />}>
          ...
          <Stack spacing={1}>
            ...
            {list('ExternalActions').map((row, index) => (
              <Stack key={`ext-${index}`} direction={{ xs: 'column', md: 'row' }} spacing={1} alignItems="center">
                <TextField
                  label="Action"
                  size="small"
                  fullWidth
                  value={row?.Action || ''}
                  onChange={(event) => patchRow('ExternalActions', index, { Action: event.target.value })}
                />
                <TextField
                  type="datetime-local"
                  label="Date"
                  ...
                  onChange={(event) =>
                    patchRow('ExternalActions', index, { DoneUtc: toUtcFromInput(event.target.value) })
                  }
                />
                <TextField
                  label="Par"
                  size="small"
                  value={row?.By || ''}
                  onChange={(event) => patchRow('ExternalActions', index, { By: event.target.value })}
                />
                <IconButton aria-label="Supprimer l'action" onClick={() => removeRow('ExternalActions', index)}>
          ...
          <Stack spacing={1}>
            ...
            {list('ThirdPartiesNotified').map((row, index) => (
              <Stack key={`tp-${index}`} direction={{ xs: 'column', md: 'row' }} spacing={1} alignItems="center">
                <TextField
                  label="Tiers"
                  size="small"
                  fullWidth
                  value={row?.Name || ''}
                  onChange={(event) => patchRow('ThirdPartiesNotified', index, { Name: event.target.value })}
                />
                <TextField
                  type="datetime-local"
                  label="Prévenu le"
                ...
                <TextField
                  ...
                  value={row?.Channel || ''}
                  onChange={(event) => patchRow('ThirdPartiesNotified', index, { Channel: event.target.value })}
                />
                <IconButton
            ...
            <Button
              size="small"
              startIcon={<Add />}
              onClick={() => addRow('ThirdPartiesNotified', { Name: '', NotifiedUtc: '', Channel: '' })}
            >
              Ajouter un tiers
            </Button>
          </Stack>
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            <Button
              ...
              onClick={handleSave}
            >
              {saveRequest.isPending ? 'Enregistrement...' : "Enregistrer la fiche d'incident"}
            </Button>
